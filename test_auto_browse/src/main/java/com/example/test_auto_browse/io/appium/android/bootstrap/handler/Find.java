...
import com.example.test_auto_browse.io.appium.android.bootstrap.exceptions.InvalidStrategyException;
import com.example.test_auto_browse.io.appium.android.bootstrap.exceptions.UiSelectorSyntaxException;
import com.example.test_auto_browse.io.appium.android.bootstrap.selector.Strategy;
import com.example.test_auto_browse.io.appium.android.bootstrap.utils.ClassInstancePair;
import com.example.test_auto_browse.io.appium.android.bootstrap.utils.ElementHelpers;
import com.example.test_auto_browse.io.appium.android.bootstrap.utils.ReflectionUtils;
import com.example.test_auto_browse.io.appium.android.bootstrap.utils.UiAutomatorParser;
...
import static com.example.test_auto_browse.io.appium.android.bootstrap.utils.API.API_18;
...
public class Find extends CommandHandler {
   ...
   *
   * @see com.example.test_auto_browse.io.appium.android.bootstrap.handler.Find # execute(io.appium.android.
   *      bootstrap.AndroidCommand)
   *
   * @param command
  ...
  private AndroidCommandResult execute(final AndroidCommand command,
    ...
    AndroidCommandResult result = executeCommand(command, isRetry);

    if (!TextUtils.isEmpty(result.toString())) {
      JSONObject jsonResult = new JSONObject(result.toString());
      JSONArray value = jsonResult.getJSONArray("value");
      int status = jsonResult.getInt("status");

      if (value.length() == 0 || status != WDStatus.SUCCESS.code()) {
        // check if we can get root xml node
        AccessibilityNodeInfo rootNode = UiAutomatorBridge.getInstance().getQueryController().getAccessibilityRootNode();
        if (null == rootNode) {
          result = new AndroidCommandResult(WDStatus.UIAUTOMATOR_GOT_NULL_ROOT_NODE_ERROR, value);
          Logger.error("Find.execute(), uiautomator got null root node!");
        }
      }
    } else {
      result = new AndroidCommandResult(WDStatus.UIAUTOMATOR_GOT_NULL_ROOT_NODE_ERROR, new JSONArray());
    }

    return result;
  }

  private AndroidCommandResult executeCommand(AndroidCommand command, boolean isRetry) throws JSONException {
    ...
    try {
      ...
      if (!multiple) {
        ...
        for (final UiSelector sel : selectors) {
          ...
          try {
            ...
            Logger.debug("-mqmsdebug", "Find.executeCommand(), UiObjectNotFoundException");
          }
        }
      ...
      if (!found) {
        if (!isRetry) {
          // reset UiAutomation connection and re-try
          Logger.debug("-mqmsdebug", "Find.executeCommand(), reset uiautomation connect and re-try");
          SysUtil.resetUiAutomationConnection();

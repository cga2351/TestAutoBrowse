...
public class AndroidElementsHash {
  ...
  public ArrayList<AndroidElement> getElements(final UiSelector sel,
    ...
    Logger.debug("-mqmsdebug","AndroidElementsHash.getElements(), waitAutoCmdRspTimeout, getElements selector:" + selectorString);
    ...
    if (endsWithInstance) {
      Logger.debug("-mqmsdebug","AndroidElementsHash.getElements(), waitAutoCmdRspTimeout, Selector ends with instance.");
    ...
    while (keepSearching) {
      if (baseEl == null) {
        Logger.debug("-mqmsdebug","AndroidElementsHash.getElements(), waitAutoCmdRspTimeout, Element[" + key + "] is null: (" + counter + ")");

        if (useIndex) {
          Logger.debug("-mqmsdebug","AndroidElementsHash.getElements(), waitAutoCmdRspTimeout, using index...");
          tmp = sel.index(counter);
        } else {
          tmp = sel.instance(counter);
        }

        Logger.debug("-mqmsdebug","AndroidElementsHash.getElements(), getElements tmp selector:" + tmp.toString());
        lastFoundObj = new UiObject(tmp);
      } else {
        Logger.debug("-mqmsdebug","AndroidElementsHash.getElements(), Element[" + key + "] is " + baseEl.getId() + ", counter: "
            + counter);
      ...
      if (lastFoundObj != null && lastFoundObj.exists()) {
        Logger.debug("-mqmsdebug", "AndroidElementsHash.getElements(), waitAutoCmdRspTimeout, lastFoundObj exist");
        elements.add(addElement(lastFoundObj));
      } else {
        Logger.debug("-mqmsdebug", "AndroidElementsHash.getElements(), waitAutoCmdRspTimeout, lastFoundObj not exist");
        keepSearching = false;
      }
    }
    return elements;

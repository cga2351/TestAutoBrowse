...
public abstract class ElementHelpers {
  ...
  private static AccessibilityNodeInfo elementToNode(AndroidElement element) {
    ...
    try {
      // don not re-try finding accessibility node info, set time out 0, java agent will re-try.
//      result = (AccessibilityNodeInfo) findAccessibilityNodeInfo.invoke(element.getUiObject(), 5000L);
      result = (AccessibilityNodeInfo) findAccessibilityNodeInfo.invoke(element.getUiObject(), 0L);
    } catch (Exception e) {
      e.printStackTrace();
    }
    return result;

...
public class UiWatchers {
  ...
  public void registerAnrAndCrashWatchers() {
      ...
      public boolean checkForCondition() {
        ...
        if (window.exists()) {
          try {
            ...
            Logger.error(LOG_TAG, "dialog gone?", e);
          }
    ...
    UiDevice.getInstance().registerWatcher("CRASH", new UiWatcher() {
      @Override
      public boolean checkForCondition() {
        Logger.debug("-mqmsdebug", "CRASH watcher checkForCondition(), entry");
        UiObject window = new UiObject(new UiSelector()
        ...
        if (window.exists()) {
          ...
          Logger.debug("-mqmsdebug", "CRASH detected, errorText=" + errorText);
    ...
    UiDevice.getInstance().registerWatcher("CRASH2", new UiWatcher() {
      @Override
      public boolean checkForCondition() {
        Logger.debug("-mqmsdebug", "CRASH2 watcher checkForCondition(), entry");
        UiObject window = new UiObject(new UiSelector().packageName("android")
        ...
        if (window.exists()) {
          ...
          Logger.debug("-mqmsdebug", "CRASH2 detected, errorText=" + errorText);
          onCrashDetected(errorText);
          postHandler();
          return true; // triggered
        }
    ...
    Logger.debug(LOG_TAG, "Registed GUI Exception watchers");
  }
  ...
  public void postHandler() {
    ...
    Logger.error(LOG_TAG, formatedOutput);
